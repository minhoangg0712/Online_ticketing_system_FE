...
export class UserManagementComponent implements OnInit {
  ...
  loadUsers(): void {
    ...
    this.adminService.getUsers(
      ...
      next: (response) => {
        console.log('Processed response in loadUsers:', JSON.stringify(response, null, 2));
        this.users = response.data.listUsers ?? [];
        this.totalItems = response.data.totalPage && response.data.pageSize
          ? response.data.totalPage * response.data.pageSize
          : 0;
  ...
  toggleSelectAll(): void {
    const paginatedUserIds = this.users.map(user => user.id);
    if (this.selectAll) {
  ...
  updateSelectAllState(): void {
    ...
    this.selectAll = currentUsers.every(user => this.selectedUsers.includes(user.id));
  }
